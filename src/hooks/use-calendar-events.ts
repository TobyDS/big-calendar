...
import { addMonths, endOfMonth, isAfter, isBefore, isEqual } from "date-fns";
...
const eventsKeys = {
  all: ["events"] as const,
  month: (year: number, month: number) => [...eventsKeys.all, "month", year, month] as const,
};
...
export function useCalendarEvents({
  ...
  const allEvents = useMemo(() => {
    ...
    const activeQueryData = monthQueries.filter(query => query.data).flatMap(query => query.data || []);
  ...
  const visibleEvents = useMemo(() => {
    return allEvents.filter((event: IDefaultEvent) => {
      ...
      const dateOverlap =
        ((isEqual(eventStart, visibleStart) || isAfter(eventStart, visibleStart)) && (isEqual(eventStart, visibleEnd) || isBefore(eventStart, visibleEnd))) ||
        ((isEqual(eventEnd, visibleStart) || isAfter(eventEnd, visibleStart)) && (isEqual(eventEnd, visibleEnd) || isBefore(eventEnd, visibleEnd))) ||
  ...
  const prefetchAdjacent = useCallback(() => {
    ...
    queryClient.prefetchQuery({
      ...
      queryFn: () => getEvents(new Date(nextYear, nextMonthNum, 1), endOfMonth(new Date(nextYear, nextMonthNum, 1))),
      staleTime: FIVE_MINUTES_IN_MS,
    });
    ...
    queryClient.prefetchQuery({
      ...
      queryFn: () => getEvents(new Date(prevYear, prevMonthNum, 1), endOfMonth(new Date(prevYear, prevMonthNum, 1))),
      staleTime: FIVE_MINUTES_IN_MS,
    });
